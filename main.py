...
class UserHandler(webapp.RequestHandler):
    def get(self):
        ...
        path = os.path.join(os.path.dirname(__file__), 'listofuser.html')
        self.response.out.write(template.render(path, values))
...
class PrintHandler(webapp.RequestHandler):
    def post(self):
        ...
        path = os.path.join(os.path.dirname(__file__), 'printuser.html')
        self.response.out.write(template.render(path, values))
...
def main():
	run_wsgi_app(webapp.WSGIApplication([
		...
		('/serve/([^/]+)?', ServeHandler),
        ('/delete', DeleteHandler),('/list', UserHandler),
        ('/print',PrintHandler)
		], debug=True))
